...
export class GetEmployeesService {
  ...
  updateEmployeementData(data:any,id:any,selectedValue:any){
    this.service.authLink();
    const httpheaders = new HttpHeaders({
      Authorization: 'Bearer ' + localStorage.getItem('token'),
    });
    ...
    return this.http.put(environment.baseApiKey + '/update_EmployementDetails/'+id+'/'+selectedValue,data, { headers: httpheaders });
  ...
  updateID(selectedId: number, sendData: any,selectedValue:any) {
    this.service.authLink();
    const httpheaders = new HttpHeaders({
      Authorization: 'Bearer ' + localStorage.getItem('token'),
    });
    const url1 = `${this.updateEmployee}/${selectedId}/${selectedValue}`;
  ...
  searchAllData(esd:any,emp_no:any,end:any){
    const httpheaders = new HttpHeaders({
      Authorization: 'Bearer ' + localStorage.getItem('token'),
    });
    const allDataSearchapi = environment.baseApiKey +'/getDetailsByEmpEsdEed/' + emp_no +'/' + esd + '/' + '4712-12-31';
    // console.log(allDataSearchapi);
    return this.http.get(allDataSearchapi, { headers: httpheaders });
...
//api for view history of emp/candi 

empSearchViewHistory(esd:any,emp_id:any,end:any){
  const httpheaders = new HttpHeaders({
    Authorization: 'Bearer ' + localStorage.getItem('token'),
  });

  const empsearchViewHistoryApi =environment.baseApiKey + '/getEmployeeDetails/' + emp_id + '/' + esd + '/' + '4712-12-31';
  console.log("empsearchViewHistoryApi",empsearchViewHistoryApi);
  return this.http.get(empsearchViewHistoryApi, { headers: httpheaders });
}

empViewHistoryData(emp_id: any) {
  const httpHeaders = new HttpHeaders({
    Authorization: 'Bearer ' + localStorage.getItem('token'),
  });
  const empviewApi = environment.baseApiKey + '/Viewhistory/'+emp_id;
  // console.log("viewApi", empviewApi);
  return this.http.get(empviewApi, { headers: httpHeaders });
 
}

//==================employeementViewHistory=============================//


EmployementViewHistoryData(emp_id:any):Observable<any>{
  const httpHeaders=new HttpHeaders({
    Authorization: 'Bearer ' + localStorage.getItem('token'),
});
const employmentviewApi = environment.baseApiKey + '/EViewhistory/' + emp_id;
console.log("employmentviewApi",employmentviewApi);
return this.http.get<any>(employmentviewApi, { headers: httpHeaders });
}
